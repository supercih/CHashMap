...
GENERICS_LIST_DEFFINE(IntList, int);
...
/************************** HashMap Node类型 ****************************/
/* HashMap的节点类型，用于存储数据。其字节点可能为空，也可能根据其所在树的层级，包含 */
/* 对应质数序列中相应数量的字节点。                                          */
/***********************************************************************/
struct _HASH_MAP_NODE {
    ...
    PHASH_MAP_NODE  NextLevelNodes;     //字节点指针

    PHASH_MAP       pChildMap;          //当到达现有树的第十层，仍然有冲突存在，则生成子HashMap
};

/****************************** HashMap类型 *****************************/
/* 为了保证通用性,不指定具体的key类型和value类型,但需要知道key类型和value类型各自占 */
/* 用的字节数,用来正确分配内存存储数据;同时保存了质数序列,使用户能够自定义质数序列长度 */
/************************************************************************/
struct _HASH_MAP {
    ...
    int             NodesCount;             //HashMap存储的元素数量（不是总节点数)
...
static inline int HashMap_Count(IN PHASH_MAP pHashMap)
{
    return pHashMap->NodesCount;
}
