...
void* HashMap_Add(IN_OUT PHASH_MAP pHashMap, IN void* pKey, IN void* pValue)
{
    ...
    if(pKey == NULL || pValue == NULL || pHashMap->ItemCount >= INT_MAX) {
        return NULL;
    }
                    ...
                    if(memcmp(pNode->Key, pKey, pHashMap->KeySizeInBytes) == 0) {
                        ...
                        if(level >= pHashMap->PrimeNumberArrayLength) {
                            ...
                            if(pNode->NextLevelNodes == NULL) {
                                pNode->NextLevelNodes = AllocateMemory(pHashMap->PrimeNumberArray[level] * sizeof(PHASH_MAP_NODE));
                                if(pNode->NextLevelNodes == NULL) {
                                    pRet = NULL;
                                    break;
                                }
                            ...
                            if(pNode->NextLevelNodes != NULL) {
                                if(pNode->NextLevelNodes[residual % pHashMap->PrimeNumberArray[level]] == NULL) {
                                    pNode->NextLevelNodes[residual % pHashMap->PrimeNumberArray[level]] = AllocateMemory(sizeof(HASH_MAP_NODE));
                                    pHashMap->TotalNodeCount++;
                                }
...
void* HashMap_PlaceHold(IN_OUT PHASH_MAP pHashMap, IN void* pKey)
{
    ...
    if(pKey == NULL || pHashMap->ItemCount >= INT_MAX) {
        return NULL;
    }
...
BOOL HashMap_Clear(IN_OUT PHASH_MAP pHashMap)
{
    ...
    while(TRUE) {
        ...
        if(pNode->pChildMap != NULL) {
            pHashMap->ItemCount -= HashMap_Count(pNode->pChildMap);
...
BOOL HashMap_Iterator_GetNext(IN PHASH_MAP_ITERATOR pIterator, OUT void* pKey, OUT void* pValue)
{
    ...
    while(TRUE) {
        if(pIterator->Counter >= pIterator->pMap->ItemCount) {
            return FALSE;
        }
